...
const GameCenter: React.FC<GameCenterProps> = ({ stats, onUpdateStats }) => {
  ...
  // SUDOKU LOGIC (Generation code remains same)
  ...
  const checkWin = (current: Board) => {
      for(let i=0; i<9; i++) {
          for(let j=0; j<9; j++) {
              ...
              const temp = [...current];
              temp[i] = [...current[i]];
              temp[i][j] = null; // Remove self to check safety
  ...
  // INTERACTION
  const startGame = () => {
      if (stats.points < COST_TO_PLAY) {
          setMessage(`Insufficient Points. Need ${COST_TO_PLAY}.`);
          return;
      }
      onUpdateStats({ ...stats, points: stats.points - COST_TO_PLAY });
  ...
  const handleNumberInput = (num: number) => {
      ...
      if (checkWin(newBoard)) {
          ...
          onUpdateStats({ ...stats, points: stats.points - COST_TO_PLAY + bonus });
      }
  };
  ...
  }, [isPlaying, isWon, selectedCell, board]);

  return (
    <div className="max-w-4xl mx-auto h-full flex flex-col items-center pb-8 px-4">
      <div className="text-center mb-6">
        <h1 className="text-3xl font-serif font-bold text-slate-900 mb-2">Scholar Sudoku</h1>
        <p className="text-slate-500">Train your logical reasoning. Relax and focus.</p>
      </div>
      ...
      {!isPlaying ? (
          <div className="bg-white p-12 rounded-2xl shadow-lg border border-slate-200 text-center max-w-lg w-full">
             ...
             <h2 className="text-2xl font-bold text-slate-900 mb-4">Start New Game</h2>
             ...
             <div className="bg-blue-50 p-4 rounded-xl mb-6">
                <p className="text-blue-900 font-bold">Cost: {COST_TO_PLAY} Points</p>
                <p className="text-blue-700 text-sm">Current Balance: {stats.points}</p>
             </div>
          ...
          <div className="flex flex-col md:flex-row gap-8 items-start">
             ...
             <div className="flex flex-col gap-6">
                 <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm w-full md:w-64">
                    {isWon ? (
                        <div className="text-center animate-in zoom-in">
                            ...
                            <button onClick={() => setIsPlaying(false)} className="mt-4 text-blue-600 underline">Play Again</button>
                        </div>
                    ) : (
                        <>
                 ...
                 <div className="text-center">
                    <button 
                        onClick={() => { if(confirm("Give up?")) setIsPlaying(false); }}
